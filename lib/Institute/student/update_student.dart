...
class _UpdateTeacherState extends ConsumerState<UpdateStudent> {
  ...
  search() async{
    ...
    if (searchValue.startsWith('S')) {
      ...
      if (response.statusCode == 200) {
        ...
        contactNumberController.text=student!.contact;
        emailController.text=student!.email;
        birthdayController.text = student!.dob.toIso8601String().split('T')[0];
  ...
  updateTeacher(BuildContext context,WidgetRef ref) async {
    ...
    if(isSearched) {
      ...
      if (validator(firstName, lastName, birthday, contact, email, address)) {
        ...
        if (isSearched) {
          ...
          var request = http.Request(
              'PATCH', Uri.parse('$studentBaseUrl/update'));
          ...
          if (response.statusCode == 200) {
            ...
            ref
                .read(loadingProvider.notifier)
                .state = false;
            showAlertDialog(context, "Success", 'Student updated successfully');
          } else {
            ref
                .read(loadingProvider.notifier)
                .state = false;
            showAlertDialog(context, "Student Not Searched",
                "Search a student before updating");
          }
        } else {
          ref.read(loadingProvider.notifier).state = false;
      ...
      ref.read(loadingProvider.notifier).state = false;
      showAlertDialog(context, "Error", "Search a student to update");
    }
  }
